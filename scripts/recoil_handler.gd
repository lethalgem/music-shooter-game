...
func _physics_process(delta):
	...
	if current_time < 1:
		...
		target_object.position.z = lerp(
			target_object.position.z, def_pos.z + target_pos.z, lerp_speed * delta
		)
		target_object.rotation.z = lerp(
			target_object.rotation.z, def_rot.z + target_rot.z, lerp_speed * delta
		)
		target_object.rotation.x = lerp(
			target_object.rotation.x, def_rot.x + target_rot.x, lerp_speed * delta
		)
